...
class MetricsStore {
  ...
  _prepare() {
    ...
    this.writeRun = this.database.transaction((timestamp, samples) => {
  ...
  getAppMetrics(appName, now) {
    ...
    `).all(appName, cutoff24h, now, appName);
    return {
      current,
